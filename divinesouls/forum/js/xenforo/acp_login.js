 ...
 */
(function(f,j,k){XenForo.AcpLoginForm=function(c){var h=f("#loginControls"),a=f("#loginLogo"),i=f("#errorMessage"),d=c.find('input[name="login"]');d.length&&d.val()==""?d.focus():c.find('input[name="password"]').focus();c.submit(function(d){d.preventDefault();a.data("width")||(a.data("width",a.width()),a.data("marginTop",a.css("marginTop")));h.xfFadeOut(XenForo.speed.normal);XenForo.ajax(c.attr("action"),c.serializeArray(),function(b){i.hide();b._redirectStatus&&b._redirectStatus=="ok"?a.animate({width:100,
