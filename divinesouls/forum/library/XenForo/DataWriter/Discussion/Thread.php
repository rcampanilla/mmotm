...
class XenForo_DataWriter_Discussion_Thread extends XenForo_DataWriter_Discussion
{
	...
	*/
	...
	protected function _updateUserMessageCount(array $messages, $isDelete = false, $forceUpdateType = null)
	{
		...
		{
			return parent::_updateUserMessageCount($messages, $isDelete, $forceUpdateType);
		}
	}
	 ...
	 */
	protected function _handleForumMove(array $messages, $sourceForumId, $destinationForumId)
	{
		...
		if ($sourceForum && $destinationForum && $sourceForum['count_messages'] != $destinationForum['count_messages'])
		{
			...
			if ($newState == 'visible' && ($oldState != 'visible' || $destinationForum['count_messages']))
			{
				...
				parent::_updateUserMessageCount($messages, false, 'add');
			}
			else if ($oldState == 'visible' && ($newState != 'visible' || !$destinationForum['count_messages']))
			{
				...
				parent::_updateUserMessageCount($messages, false, 'subtract');
			}
		}
	}
